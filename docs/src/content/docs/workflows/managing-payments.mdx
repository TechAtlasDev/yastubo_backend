...
Cuando se crea una suscripción a través de un revendedor, se aplica una `application_fee_percent` (tasa de comisión) definida en el `Workspace` del socio.
...
async def create_subscription(
    ...
    # 1. Obtener la póliza y el espacio de trabajo (Workspace)
    ...
    connect_account_id = workspace.stripe_connect_id if workspace.is_reseller else None
    app_fee = float(workspace.commission_rate) if workspace.commission_rate > 0 else None
