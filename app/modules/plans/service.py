...
async def create_plan(
    db: AsyncSession, data: PlanCreate, created_by: uuid.UUID
) -> Plan:
    plan = Plan(
        name=data.name,
        ...
        vesting_accidental_days=data.vesting_accidental_days,
        vesting_natural_days=data.vesting_natural_days,
        vesting_suicide_days=data.vesting_suicide_days,
    )
    db.add(plan)
    await db.flush()
    ...
    for cc in data.country_configs:
        country_config = CountryConfig(
            ...
            base_price_override=float(cc.base_price_override)
    ...
    snapshot = {
        ...
        "vesting": {
            "accidental": plan.vesting_accidental_days,
            "natural": plan.vesting_natural_days,
            "suicide": plan.vesting_suicide_days,
        },
    }

    version = PlanVersion(
        plan_id=plan.id, version_number=1, snapshot=snapshot, created_by=created_by
    )
    db.add(version)
...
async def get_plan(db: AsyncSession, plan_id: uuid.UUID) -> Plan:
    ...
    plan.current_version = (
        max([v.version_number for v in plan.versions]) if plan.versions else 1
    )
    return plan
...
async def list_plans(db: AsyncSession, active_only: bool = True) -> List[Plan]:
    ...
    for p in plans:
        p.current_version = (
            max([v.version_number for v in p.versions]) if p.versions else 1
        )
    return list(plans)
...
async def update_plan(
    db: AsyncSession, plan_id: uuid.UUID, data: PlanUpdate, updated_by: uuid.UUID
) -> Plan:
    ...
    for field in [
        ...
        "terms_es",
        "terms_en",
        "vesting_accidental_days",
        "vesting_natural_days",
        "vesting_suicide_days",
    ]:
    ...
    if "coverage_ids" in update_data:
        # Clear association
        await db.execute(
            PlanCoverage.__table__.delete().where(PlanCoverage.plan_id == plan.id)
        )
        for cov_id in data.coverage_ids:
    ...
    # New Version
    next_version = (
        max([v.version_number for v in plan.versions]) if plan.versions else 0
    ) + 1
    snapshot = {
        "name": plan.name,
        "base_price": float(plan.base_price),
...
async def toggle_plan(
    db: AsyncSession, plan_id: uuid.UUID, updated_by: uuid.UUID
) -> Plan:
    plan = await get_plan(db, plan_id)
...
async def get_price(
    ...
    # Validate country
    config = next(
        (cc for cc in plan.country_configs if cc.country_code == data.country_code),
        None,
    )
    ...
    if data.age > plan.max_entry_age:
        raise HTTPException(
            status_code=422,
            detail=f"Age {data.age} exceeds max entry age {plan.max_entry_age}",
        )
