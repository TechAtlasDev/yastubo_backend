...
async def get_dashboard_metrics(db: AsyncSession, workspace_id: uuid.UUID) -> DashboardKPIMetrics:
    # 1. Total Revenue
    ...
    churn_rate = (cancelled_count / active_policies * 100) if active_policies > 0 else 0.0

    # 5. LTV Average
    clients_stmt = select(func.count(func.distinct(Policy.client_id))).where(Policy.workspace_id == workspace_id)
    ...
    channels_stmt = select(Lead.source_channel, func.count(Lead.id)).where(
        and_(
            Lead.workspace_id == workspace_id,
            Lead.purchase_completed == True
        )
    ...
    # 8. Top Plans
    top_plans_stmt = select(Policy.plan_version_snapshot['name'], func.count(Policy.id)).where(
        Policy.workspace_id == workspace_id
    ).group_by(Policy.plan_version_snapshot['name']).order_by(func.count(Policy.id).desc()).limit(5)
