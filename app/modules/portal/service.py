...
async def get_client_by_user_email(db: AsyncSession, email: str) -> Client:
    ...
    if not client:
        raise HTTPException(
            status_code=404, detail="Client profile not found for this user"
        )
    return client
...
async def get_client_policy_detail(
    db: AsyncSession, policy_id: uuid.UUID, client_id: uuid.UUID
) -> Policy:
...
async def cancel_policy_by_client(
    db: AsyncSession, policy_id: uuid.UUID, client_id: uuid.UUID, reason: str
) -> Policy:
    ...
    if policy.status not in allowed_statuses:
        raise HTTPException(
            status_code=400, detail=f"Cannot cancel policy with status {policy.status}"
        )
...
async def add_payment_method(
    db: AsyncSession, stripe_client: StripeClient, user_id: uuid.UUID, stripe_pm_id: str
) -> PaymentMethod:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid Stripe Payment Method: {str(e)}"
        )
...
async def pay_pending_policy(
    ...
    if policy.status not in [PolicyStatus.IN_ARREARS, PolicyStatus.PENDING_PAYMENT]:
        raise HTTPException(
            status_code=400, detail="Policy does not have pending payments or arrears"
        )
    ...
    if not pm:
        raise HTTPException(
            status_code=400,
            detail="No default payment method found. Please add one first.",
        )

    customer_id = await stripe_client.get_customer_id_by_email(client.email)
    if not customer_id:
        customer = await stripe_client.create_customer(
            email=client.email, name=f"{client.first_name} {client.last_name}"
        )
