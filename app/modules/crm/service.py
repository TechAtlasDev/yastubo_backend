...
async def sync_client_to_crm(zoho, client) -> str | None:
    try:
        ...
        logger.info("[CRM] sync_client email={} contact_id={}", client.email, contact_id)
        return contact_id
...
async def sync_policy_to_crm(zoho, policy, client, plan_name: str) -> str | None:
    try:
        ...
        logger.info("[CRM] sync_policy policy={} deal_id={}", policy.policy_number, deal_id)
        return deal_id
    except Exception as exc:
...
async def update_policy_stage_in_crm(zoho, policy) -> bool:
    try:
        ...
        if not deal_id:
            logger.info("[CRM] no deal_id for policy={}, skipping stage update", policy.policy_number)
            return False
        stage = policy_status_to_zoho_stage(policy.status)
        ok = await zoho.update_deal_stage(deal_id=deal_id, stage=stage)
        logger.info("[CRM] update_stage policy={} stage={} ok={}", policy.policy_number, stage, ok)
        return ok
    except Exception as exc:
