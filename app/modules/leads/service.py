...
async def create_or_update_lead(db: AsyncSession, data: LeadCreate) -> Lead:
    ...
    if lead:
        ...
        dispatch_event_background("LEAD_UPDATED", {"lead_id": str(lead.id), "phone": lead.phone_e164})
    else:
        ...
        dispatch_event_background("LEAD_CREATED", {"lead_id": str(lead.id), "phone": lead.phone_e164})
...
async def list_leads(db: AsyncSession, status: Optional[LeadStatus] = None) -> List[Lead]:
