...
class Client(BaseModel):
    ...
    # Retention & Risk
    churn_risk_level: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # LOW, MEDIUM, HIGH, CRITICAL
    collections_risk_level: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
    last_failed_payment_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )
    retention_sequence_status: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
...
class Policy(BaseModel):
    ...
    status_history: Mapped[List["PolicyStatusHistory"]] = relationship(
        "PolicyStatusHistory", back_populates="policy", cascade="all, delete-orphan"
    )
    transactions: Mapped[List["Transaction"]] = relationship(
        "Transaction", back_populates="policy"
    )
...
class Beneficiary(BaseModel):
    ...
    kinship_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # SELF, SPOUSE, CHILD, PARENT, OTHER
    country_of_residence: Mapped[str] = mapped_column(String(2), nullable=False)
    location_type: Mapped[str] = mapped_column(
        String(50), default="URBAN"
    )  # URBAN, RURAL
    ...
    deceased_reported_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )
    deceased_reported_by: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True
    )
    backend_api_call_sent: Mapped[bool] = mapped_column(Boolean, default=False)
    backend_api_call_sent_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )
    billing_adjustment_confirmed: Mapped[bool] = mapped_column(Boolean, default=False)

    # Sync
    zoho_beneficiary_id: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True
    )
