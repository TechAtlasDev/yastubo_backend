...
async def test_duplicate_lead_updates_existing(client: AsyncClient):
    ...
    admin_token = create_access_token({"sub": str(uuid.uuid4()), "roles": ["ADMIN"], "type": "access"})
...
async def test_update_lead_checkout_tracking(client: AsyncClient, admin_user):
    ...
    response = await client.patch(f"/api/v1/leads/{lead_id}", json=update_payload, headers=headers)
    ...
    response = await client.patch(f"/api/v1/leads/{lead_id}", json=purchase_payload, headers=headers)
    assert response.status_code == 200
    data = response.json()
