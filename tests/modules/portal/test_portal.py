...
@pytest.mark.asyncio
async def test_client_can_see_own_policies(
    client: AsyncClient, client_user_token: str, active_policy_for_client: Policy
):
...
@pytest.mark.asyncio
async def test_client_cannot_see_other_client_policies(
    client: AsyncClient,
    admin_token: str,
...
@pytest.mark.asyncio
async def test_client_can_cancel_active_policy(
    client: AsyncClient, client_user_token: str, active_policy_for_client: Policy
):
...
@pytest.mark.asyncio
async def test_client_cannot_cancel_closed_case_policy(
    client: AsyncClient,
    client_user_token: str,
    db_session: AsyncSession,
    active_policy_for_client: Policy,
):
...
async def test_client_can_add_payment_method(
    mock_stripe, client: AsyncClient, client_user_token: str
):
    ...
    with patch(
        "app.modules.payments.stripe_client.StripeClient",
        return_value=mock_stripe_instance,
    ):
        response = await client.post(
...
@pytest.mark.asyncio
async def test_client_can_set_default_payment_method(
    client: AsyncClient,
    client_user_token: str,
    client_user_with_profile,
    db_session: AsyncSession,
):
...
async def test_client_can_pay_pending_policy(
    ...
    mock_stripe_instance.create_payment_intent.return_value = {
        "id": "pi_123",
        "status": "succeeded",
    }
    ...
    res = await db_session.execute(
        select(Transaction).where(Transaction.policy_id == active_policy_for_client.id)
    )
    tx = res.scalar_one()
...
@pytest.mark.asyncio
async def test_client_without_profile_returns_404(
    client: AsyncClient, db_session: AsyncSession, roles
):
    # 1. Create a user without a client profile
    email = f"no_profile_{uuid.uuid4().hex[:6]}@example.com"
    user = User(
        email=email, hashed_password="hashed", full_name="No Profile", is_active=True
    )
    db_session.add(user)
    ...
    token = create_access_token(
        {"sub": str(user.id), "email": email, "roles": ["CLIENTE"], "type": "access"}
    )
