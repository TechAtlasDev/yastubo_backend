...
async def test_login_valid_credentials_returns_tokens(client: AsyncClient, client_user):
    payload = {"email": client_user.email, "password": "password123"}
...
async def test_refresh_tokens_returns_new_pair(
    client: AsyncClient, client_user, redis_client
):
...
@pytest.mark.asyncio
async def test_assign_role_by_admin_succeeds(
    client: AsyncClient, admin_user, client_user, roles
):
    token = create_access_token(
        {
    ...
    headers = {"Authorization": f"Bearer {token}"}
    payload = {"user_id": str(client_user.id), "role_name": "VENDEDOR"}
    response = await client.post(
...
@pytest.mark.asyncio
async def test_assign_role_by_non_admin_returns_403(
    client: AsyncClient, client_user, roles
):
    token = create_access_token(
        {
            "sub": str(client_user.id),
    ...
    headers = {"Authorization": f"Bearer {token}"}
    payload = {"user_id": str(client_user.id), "role_name": "ADMIN"}
    response = await client.post(
        "/api/v1/auth/roles/assign", json=payload, headers=headers
    )
