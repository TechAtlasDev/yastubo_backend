...
@pytest.mark.asyncio
async def test_register_duplicate_client_email_returns_409(
    client: AsyncClient, admin_user, test_client
):
    token = create_access_token(
        {"sub": str(admin_user.id), "roles": ["ADMIN"], "type": "access"}
    )
...
@pytest.mark.asyncio
async def test_issue_policy_returns_201_with_policy_number(
    client: AsyncClient, admin_user, emission_request_payload
):
    token = create_access_token(
        {"sub": str(admin_user.id), "roles": ["ADMIN"], "type": "access"}
    )
...
@pytest.mark.asyncio
async def test_issue_policy_age_exceeds_max_entry_age_returns_422(
    client: AsyncClient, admin_user, test_client, created_plan, db_session
):
...
@pytest.mark.asyncio
async def test_issue_policy_invalid_country_returns_422(
    client: AsyncClient, admin_user, test_client, created_plan
):
    token = create_access_token(
        {"sub": str(admin_user.id), "roles": ["ADMIN"], "type": "access"}
    )
...
@pytest.mark.asyncio
async def test_transition_policy_to_active_succeeds(
    ...
    headers = {"Authorization": f"Bearer {token}"}
    payload = {"target_status": "ACTIVE", "reason": "Payment confirmed"}
    response = await client.post(
        f"/api/v1/emission/policies/{issued_policy['id']}/transition",
        json=payload,
        headers=headers,
    )
...
@pytest.mark.asyncio
async def test_transition_policy_invalid_returns_422(
    client: AsyncClient, admin_user, issued_policy
):
    token = create_access_token(
        {"sub": str(admin_user.id), "roles": ["ADMIN"], "type": "access"}
