...
def test_valid_transition_draft_to_pending_payment():
    assert transition(PolicyStatus.DRAFT, PolicyStatus.PENDING_PAYMENT) == PolicyStatus.PENDING_PAYMENT

def test_valid_transition_pending_payment_to_active():
    assert transition(PolicyStatus.PENDING_PAYMENT, PolicyStatus.ACTIVE) == PolicyStatus.ACTIVE
